...
public class CubeMainHD {
    ...
    public static void main(String[] args) {
        System.out.println("starting CubeMainHD Infinity menu-prototyp...");
        CubeMainHD main = new CubeMainHD();
        try {
            main.start();
        } catch (Exception e) {
    ...
    private void start() throws Exception {
        ...
        nifty.fromXml("resources/ui/Toolbar.xml", "Screen1");
        showUI = false;
        ...
        DropDown dpd_Shadertype = nifty.getCurrentScreen().findNiftyControl("dpd_Shadertype", DropDown.class);
        if (dpd_Shadertype != null) {
            dpd_Shadertype.addAllItems(Arrays.asList(pnames));
        }
        else {
            System.out.println("ERROR no DropBox found!");
        }

        DropDown dpd_Optitype = nifty.getCurrentScreen().findNiftyControl("dpd_Optitype", DropDown.class);
        if (dpd_Optitype != null) {
            dpd_Optitype.addItem("Opt1");
            dpd_Optitype.addItem("Opt2");
            dpd_Optitype.addItem("Opt3");
            dpd_Optitype.addItem("Opt4");
        }
        else {
            System.out.println("ERROR no DropBox found!");
        }

        CheckBox chb_polygonmode = nifty.getCurrentScreen().findNiftyControl("chb_polygonmode", CheckBox.class);
        if (chb_polygonmode != null) {
            if (polygonmode) {
                chb_polygonmode.check();
            }
            else {
                chb_polygonmode.uncheck();
            }
        }
        else {
            System.out.println("ERROR no CheckBox found!");
        }

        ImageSelect img_texture = nifty.getCurrentScreen().findNiftyControl("img_texture", ImageSelect.class);
        if (img_texture != null) {
            NiftyImage NiftyImage1 = nifty.getRenderEngine().createImage("resources/images/grass.png", false);
            NiftyImage NiftyImage2 = nifty.getRenderEngine().createImage("resources/images/grass_128.png", false);
            NiftyImage NiftyImage3 = nifty.getRenderEngine().createImage("resources/images/moss_128.png", false);
            NiftyImage NiftyImage4 = nifty.getRenderEngine().createImage("resources/images/ground_stone_128.png", false);
            NiftyImage NiftyImage5 = nifty.getRenderEngine().createImage("resources/images/dirt_128.png", false);

            img_texture.addImage(NiftyImage1);
            img_texture.addImage(NiftyImage2);
            img_texture.addImage(NiftyImage3);
            img_texture.addImage(NiftyImage4);
            img_texture.addImage(NiftyImage5);
        }
        else {
            System.out.println("ERROR no CheckBox found!");
        }
    ...
    @NiftyEventSubscriber(id="dpd_Shadertype")
